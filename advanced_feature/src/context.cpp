...
bool Context::Init(){
    ...
    m_grassTexture = Texture::CreateFromImage(Image::Load("./image/12_grass.png").get());
...
void Context::Render(){
    ...
    if(ImGui::Begin("ui window")){
        ...
        ImGui::Image(m_framebuffer->GetColorAttachment()->Get(),
    ...
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT |GL_STENCIL_BUFFER_BIT);//glClearColor의 색상으로 버퍼 초기화
    ...
    //m_light.position = m_cameraPos;// 공포게임 손전등 1인칭 시점
    //m_light.direction = m_cameraFront;// 실습해보고 싶으면 lightmodeltransform ~ glDrawElements 주석

	auto projection = glm::perspective(glm::radians(45.0f),(float)m_width/(float)m_height,0.01f,100.0f);//마지막 parameter 크게 -> 회전할때 사라지는 큐브 문제 해결 가능
