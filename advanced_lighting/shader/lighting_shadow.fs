...
float ShadowCalculation(vec4 fragPosLight, vec3 normal, vec3 lightDir){
    ...
    vec3 projCoords = fragPosLight.xyz / fragPosLight.w; // projCoords -1 ~ +1
    ...
    //check whether current frag pos is in window
    float bias = max(0.015 * (1.0 - dot(normal, lightDir)), 0.0005);
    float shadow = 0.0;
